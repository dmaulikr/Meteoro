...
class GameScene: SKScene, SKPhysicsContactDelegate {
  ...
  override func didMove(to view: SKView) {
    ...
    let yOffset = (frame.height - blockHeight ) / CGFloat(numberOfRows)
    // 3.
    
    let colorList = ["block.png", "red.png","blue.png","orange.png"]
    
    for r in 1..<numberOfRows {
      let rowH = (0.3 * CGFloat(r))
        
        for i in 0..<numberOfBlocks {
          let blockColorIdx = arc4random_uniform(UInt32(colorList.count))
          let blockColor = colorList[Int(blockColorIdx)]
          print("Random Block ->\(blockColor)")
          let block = SKSpriteNode(imageNamed: blockColor)
          let yPos = (yOffset * rowH)  + (frame.height  / 1.5)
    ...
    if gameState.currentState is Playing {
      ...
      if firstBody.categoryBitMask == BallCategory && secondBody.categoryBitMask == BlockCategory {
        ...
        guard secondBody.node != nil else{
            print("Block Node Null Detected....")
            return
        }
